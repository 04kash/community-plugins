...
import { morguePlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [morguePlugin],
    });

    await request(server).get('/api/morgue/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/morgue/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/morgue/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/morgue/todos/${createdTodoItem.id}`)
